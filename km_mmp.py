...
class Kmeans_MaMiPot:
    def __init__(self, clf, sma=None, alfa_p=0.3, alfa_n=0.8, gamma=1, beta=0, p_clusters=1, n_clusters=3):
        ...
        p_clusters (int) : number of positive clusters
        n_clusters (int) : number of negative clusters
        '''
    ...
    def fit_resample(self, tr_data, tr_labels):
        ...
        if stp.shape[0] > (self.gamma + self.p_clusters) and sfn.shape[0] > 0:
            kmeans_tp = KMeans(init='k-means++', n_clusters=self.p_clusters, random_state=0, n_init=1).fit(stp)
            clusters_fn = kmeans_tp.predict(sfn) # assign false negative samples to its closest cluster

        if stn.shape[0] > (self.gamma + self.n_clusters) and sfp.shape[0] > 0:
            kmeans_tn = KMeans(init='k-means++', n_clusters=self.n_clusters, random_state=0, n_init=1).fit(stn)
            clusters_fp = kmeans_tn.predict(sfp) # assign false positive samples to its closest cluster
        ...
        while iter < 3:
            ...
            if stp.shape[0] > (self.gamma + self.p_clusters) and sfn.shape[0] > 0:
                for i, x in enumerate(sfn):
                    # move false negative samples toward their closest cluster
                    mu_p = kmeans_tp.cluster_centers_[clusters_fn[i]]
                    sfn[i] = (self.alfa_p * mu_p) + ((1 - self.alfa_p) * x)

            if stn.shape[0] > (self.gamma + self.n_clusters) and sfp.shape[0] > 0:
                for i, y in enumerate(sfp):
                    # move false positive samples toward their closest cluster
                    mu_n = kmeans_tn.cluster_centers_[clusters_fp[i]]
